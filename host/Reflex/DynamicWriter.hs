...
-- match.
instance (MonadAdjust t m, MonadFix m, Monoid w, MonadHold (Impl t) m, HasTimeline t) => MonadAdjust t (DynamicWriterT t w m) where
...
withDynamicWriterT :: (Monoid w, Monoid w', HasTimeline t, MonadHold (Impl t) m, MonadFix m)
                   => (w -> w')
