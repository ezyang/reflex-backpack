...
instance (HasTimeline t, Monad m, Monoid w) => EventWriter t w (EventWriterT t w m) where
  tellEvent w = EventWriterT $ modify (|> w)

instance MonadTrans (EventWriterT t w) where
  lift = EventWriterT . lift

instance MonadSample (Impl t) m => MonadSample (Impl t) (EventWriterT t w m) where
  sample = lift . sample

instance MonadHold (Impl t) m => MonadHold (Impl t) (EventWriterT t w m) where
  hold v0 = lift . hold v0
  holdDyn v0 = lift . holdDyn v0
  holdIncremental v0 = lift . holdIncremental v0

instance (HasTimeline t, MonadAdjust t m, MonadHold (Impl t) m, Monoid w, Semigroup w) => MonadAdjust t (EventWriterT t w m) where
