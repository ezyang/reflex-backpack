...
instance (Monad m, MonadRef m, Ref m ~ Ref IO, MonadReflexCreateTrigger t m) => TriggerEvent (Impl t) (TriggerEventT t m) where
...
instance MonadSample (Impl t) m => MonadSample (Impl t) (TriggerEventT t m) where
  {-# INLINABLE sample #-}
  sample = lift . sample

instance MonadHold (Impl t) m => MonadHold (Impl t) (TriggerEventT t m) where
  {-# INLINABLE hold #-}
