...
coerceBehavior :: (HasTimeline t, Coercible a b) => Behavior (Impl t) a -> Behavior (Impl t) b
...
coerceEvent :: (HasTimeline t, Coercible a b) => Event (Impl t) a -> Event (Impl t) b
coerceEvent = coerceWith $ eventCoercion Coercion

-- | Coerce a 'Dynamic' between representationally-equivalent value types
coerceDynamic :: (HasTimeline t, Coercible a b) => Dynamic (Impl t) a -> Dynamic (Impl t) b
...
unsafeDynamic :: HasTimeline t => Behavior (Impl t) a -> Event (Impl t) a -> Dynamic (Impl t) a
unsafeDynamic = unsafeBuildDynamic . sample

-- | Construct a 'Dynamic' value that never changes
constDyn :: HasTimeline t => a -> Dynamic (Impl t) a
constDyn = pure
...
pushAlways :: HasTimeline t => (a -> PushM t b) -> Event (Impl t) a -> Event (Impl t) b
...
instance HasTimeline t => Applicative (Behavior (Impl t)) where
  pure = constant
  f <*> x = pull $ sample f `ap` sample x
  _ *> b = b
  a <* _ = a

instance HasTimeline t => Apply (Behavior (Impl t)) where
  (<.>) = (<*>)

instance HasTimeline t => Bind (Behavior (Impl t)) where
  (>>-) = (>>=)

instance (HasTimeline t, Fractional a) => Fractional (Behavior (Impl t) a) where
  (/) = liftA2 (/)
  fromRational = pure . fromRational
  recip = fmap recip

instance HasTimeline t => Functor (Behavior (Impl t)) where
  fmap f = pull . fmap f . sample

instance (HasTimeline t, IsString a) => IsString (Behavior (Impl t) a) where
  fromString = pure . fromString

instance HasTimeline t => Monad (Behavior (Impl t)) where
  ...
  return = constant
  fail = error "Monad (Behavior (Impl t)) does not support fail"

instance (HasTimeline t, Monoid a) => Monoid (Behavior (Impl t) a) where
  mempty = constant mempty
  mappend a b = pull $ liftM2 mappend (sample a) (sample b)
  mconcat = pull . fmap mconcat . mapM sample

instance (HasTimeline t, Num a) => Num (Behavior (Impl t) a) where
  (+) = liftA2 (+)
  (-) = liftA2 (-)
  (*) = liftA2 (*)
  abs = fmap abs
...
instance (HasTimeline t, Semigroup a) => Semigroup (Behavior (Impl t) a) where
...
-- occurrence).
instance HasTimeline t => Alt (Event (Impl t)) where
  ev1 <!> ev2 = leftmost [ev1, ev2]

-- | 'Event' intersection (convenient interface to 'coincidence').
instance HasTimeline t => Apply (Event (Impl t)) where
  evf <.> evx = coincidence (fmap (<$> evx) evf)

-- | 'Event' intersection (convenient interface to 'coincidence').
instance HasTimeline t => Bind (Event (Impl t)) where
  evx >>- f = coincidence (f <$> evx)
  join = coincidence

instance HasTimeline t => Functor (Event (Impl t)) where
  {-# INLINE fmap #-}
  fmap f = fmapMaybe $ Just . f

instance HasTimeline t => FunctorMaybe (Event (Impl t)) where
  {-# INLINE fmapMaybe #-}
  fmapMaybe f = push $ return . f

-- | Never: @'zero' = 'never'@.
instance HasTimeline t => Plus (Event (Impl t)) where
  zero = never

-- | Replace each occurrence value of the 'Event' with the value of the
-- 'Behavior' at the time of that occurrence.
tag :: HasTimeline t => Behavior (Impl t) b -> Event (Impl t) a -> Event (Impl t) b
...
attach :: HasTimeline t => Behavior (Impl t) a -> Event (Impl t) b -> Event (Impl t) (a, b)
attach = attachWith (,)

-- | Create a new 'Event' that occurs when the supplied 'Event' occurs by
-- combining it with the current value of the 'Behavior'.
attachWith :: HasTimeline t => (a -> b -> c) -> Behavior (Impl t) a -> Event (Impl t) b -> Event (Impl t) c
...
-- Nothing
attachWithMaybe :: HasTimeline t => (a -> b -> Maybe c) -> Behavior (Impl t) a -> Event (Impl t) b -> Event (Impl t) c
attachWithMaybe f b e = flip push e $ \o -> (`f` o) <$> sample b

-- | Create a new 'Event' that only occurs only once, on the first occurence of
-- the supplied 'Event'.
headE :: (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => Event (Impl t) a -> m (Event (Impl t) a)
headE e = do
...
-- supplied 'Event'.
tailE :: (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => Event (Impl t) a -> m (Event (Impl t) a)
...
-- occurence.
headTailE :: (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => Event (Impl t) a -> m (Event (Impl t) a, Event (Impl t) a)
headTailE e = do
  eHead <- headE e
...
splitE :: HasTimeline t => Event (Impl t) (a, b) -> (Event (Impl t) a, Event (Impl t) b)
...
traceEvent :: (HasTimeline t, Show a) => String -> Event (Impl t) a -> Event (Impl t) a
...
-- Note: As with Debug.Trace.trace, the message will only be printed if the
-- 'Event' is actually used.
traceEventWith :: HasTimeline t => (a -> String) -> Event (Impl t) a -> Event (Impl t) a
traceEventWith f = push $ \x -> trace (f x) $ return $ Just x

instance (Semigroup a, HasTimeline t) => Semigroup (Event (Impl t) a) where
...
instance (Semigroup a, HasTimeline t) => Monoid (Event (Impl t) a) where
  mempty = never
  mappend = (<>)
...
{-# INLINE mergeWith #-}
mergeWith :: HasTimeline t => (a -> a -> a) -> [Event (Impl t) a] -> Event (Impl t) a
mergeWith = mergeWith' id

{-# INLINE mergeWith' #-}
mergeWith' :: HasTimeline t => (a -> b) -> (b -> b -> b) -> [Event (Impl t) a] -> Event (Impl t) b
...
-- leftmost event.
{-# INLINE leftmost #-}
leftmost :: HasTimeline t => [Event (Impl t) a] -> Event (Impl t) a
leftmost = mergeWith const

-- | Create a new 'Event' that occurs if at least one of the 'Event's in the
-- list occurs and has a list of the values of all 'Event's occuring at that
-- time.
mergeList :: HasTimeline t => [Event (Impl t) a] -> Event (Impl t) (NonEmpty a)
mergeList [] = never
...
mergeMap :: (HasTimeline t, Ord k) => Map k (Event (Impl t) a) -> Event (Impl t) (Map k a)
mergeMap = fmap dmapToMap . merge . mapWithFunctorToDMap

-- | Split the event into separate events for 'Left' and 'Right' values.
fanEither :: HasTimeline t => Event (Impl t) (Either a b) -> (Event (Impl t) a, Event (Impl t) b)
fanEither e =
...
fanThese :: HasTimeline t => Event (Impl t) (These a b) -> (Event (Impl t) a, Event (Impl t) b)
fanThese e =
...
fanMap :: (HasTimeline t, Ord k) => Event (Impl t) (Map k a) -> EventSelector (Impl t) (Const2 k a)
...
switchPromptly :: (HasTimeline t, MonadHold (Impl t) m) => Event (Impl t) a -> Event (Impl t) (Event (Impl t) a) -> m (Event (Impl t) a)
...
-- old value.
switchPromptOnly :: (HasTimeline t, MonadHold (Impl t) m) => Event (Impl t) a -> Event (Impl t) (Event (Impl t) a) -> m (Event (Impl t) a)
switchPromptOnly e0 e' = do
  eLag <- switch <$> hold e0 e'
  return $ coincidence $ leftmost [e', eLag <$ eLag]

instance HasTimeline t => Align (Event (Impl t)) where
  nil = never
...
gate :: HasTimeline t => Behavior (Impl t) Bool -> Event (Impl t) a -> Event (Impl t) a
...
switcher :: (HasTimeline t, MonadHold (Impl t) m)
        => Behavior (Impl t) a -> Event (Impl t) (Behavior (Impl t) a) -> m (Behavior (Impl t) a)
...
zipDyn :: HasTimeline t => Dynamic (Impl t) a -> Dynamic (Impl t) b -> Dynamic (Impl t) (a, b)
zipDyn = zipDynWith (,)
...
zipDynWith :: HasTimeline t => (a -> b -> c) -> Dynamic (Impl t) a -> Dynamic (Impl t) b -> Dynamic (Impl t) c
zipDynWith f da db =
...
instance (HasTimeline t, Monoid a) => Monoid (Dynamic (Impl t) a) where
...
distributeDMapOverDynPure :: forall t k. (HasTimeline t, GCompare k) => DMap k (Dynamic (Impl t)) -> Dynamic (Impl t) (DMap k Identity)
...
distributeListOverDyn :: HasTimeline t => [Dynamic (Impl t) a] -> Dynamic (Impl t) [a]
...
distributeListOverDynWith :: HasTimeline t => ([a] -> b) -> [Dynamic (Impl t) a] -> Dynamic (Impl t) b
...
difference :: HasTimeline t => Event (Impl t) a -> Event (Impl t) b -> Event (Impl t) a
...
alignWithMaybe
  :: HasTimeline t => (These a b -> Maybe c) -> Event (Impl t) a -> Event (Impl t) b -> Event (Impl t) c
...
class HasTimeline t => Accumulator t f | f -> t where
  accum :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> a) -> a -> Event (Impl t) b -> m (f a)
  accum f = accumMaybe $ \v o -> Just $ f v o
  accumM :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> PushM t a) -> a -> Event (Impl t) b -> m (f a)
  accumM f = accumMaybeM $ \v o -> Just <$> f v o
  accumMaybe :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> Maybe a) -> a -> Event (Impl t) b -> m (f a)
  accumMaybe f = accumMaybeM $ \v o -> return $ f v o
  accumMaybeM :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> PushM t (Maybe a)) -> a -> Event (Impl t) b -> m (f a)
  mapAccum :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> (a, c)) -> a -> Event (Impl t) b -> m (f a, Event (Impl t) c)
  mapAccum f = mapAccumMaybe $ \v o -> bimap Just Just $ f v o
  mapAccumM :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> PushM t (a, c)) -> a -> Event (Impl t) b -> m (f a, Event (Impl t) c)
  mapAccumM f = mapAccumMaybeM $ \v o -> bimap Just Just <$> f v o
  mapAccumMaybe :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> (Maybe a, Maybe c)) -> a -> Event (Impl t) b -> m (f a, Event (Impl t) c)
  mapAccumMaybe f = mapAccumMaybeM $ \v o -> return $ f v o
  mapAccumMaybeM :: (MonadHold (Impl t) m, MonadFix m) => (a -> b -> PushM t (Maybe a, Maybe c)) -> a -> Event (Impl t) b -> m (f a, Event (Impl t) c)

-- | Accumulate occurrences of an 'Event', producing an output occurrence each
-- time.  Discard the underlying 'Accumulator'.
mapAccum_ :: forall t m a b c. (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => (a -> b -> (a, c)) -> a -> Event (Impl t) b -> m (Event (Impl t) c)
mapAccum_ f z e = do
  (_ :: Behavior (Impl t) a, result) <- mapAccum f z e
  return result

-- | Accumulate occurrences of an 'Event', possibly producing an output
-- occurrence each time.  Discard the underlying 'Accumulator'.
mapAccumMaybe_ :: forall t m a b c. (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => (a -> b -> (Maybe a, Maybe c)) -> a -> Event (Impl t) b -> m (Event (Impl t) c)
mapAccumMaybe_ f z e = do
  (_ :: Behavior (Impl t) a, result) <- mapAccumMaybe f z e
  return result
...
mapAccumM_ :: forall t m a b c. (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => (a -> b -> PushM t (a, c)) -> a -> Event (Impl t) b -> m (Event (Impl t) c)
mapAccumM_ f z e = do
  (_ :: Behavior (Impl t) a, result) <- mapAccumM f z e
  return result
...
-- 'Accumulator'.
mapAccumMaybeM_ :: forall t m a b c. (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => (a -> b -> PushM t (Maybe a, Maybe c)) -> a -> Event (Impl t) b -> m (Event (Impl t) c)
mapAccumMaybeM_ f z e = do
  (_ :: Behavior (Impl t) a, result) <- mapAccumMaybeM f z e
  return result

instance HasTimeline t => Accumulator t (Dynamic (Impl t)) where
...
instance HasTimeline t => Accumulator t (Behavior (Impl t)) where
  accumMaybeM f z e = current <$> accumMaybeM f z e
  mapAccumMaybeM f z e = first current <$> mapAccumMaybeM f z e

instance HasTimeline t => Accumulator t (Event (Impl t)) where
...
zipListWithEvent :: (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => (a -> b -> c) -> [a] -> Event (Impl t) b -> m (Event (Impl t) c)
...
class (HasTimeline t, Monad m) => MonadAdjust t m | m -> t where
  runWithReplace :: m a -> Event (Impl t) (m b) -> m (a, Event (Impl t) b)
  sequenceDMapWithAdjust :: GCompare k => DMap k m -> Event (Impl t) (PatchDMap k m) -> m (DMap k Identity, Event (Impl t) (PatchDMap k Identity))
...
pushAlwaysCheap :: HasTimeline t => (a -> PushM t b) -> Event (Impl t) a -> Event (Impl t) b
pushAlwaysCheap f = pushCheap (fmap Just . f)

{-# INLINE fmapMaybeCheap #-}
fmapMaybeCheap :: HasTimeline t => (a -> Maybe b) -> Event (Impl t) a -> Event (Impl t) b
fmapMaybeCheap f = pushCheap $ return . f

{-# INLINE fforMaybeCheap #-}
fforMaybeCheap :: HasTimeline t => Event (Impl t) a -> (a -> Maybe b) -> Event (Impl t) b
fforMaybeCheap = flip fmapMaybeCheap

{-# INLINE fforCheap #-}
fforCheap :: HasTimeline t => Event (Impl t) a -> (a -> b) -> Event (Impl t) b
fforCheap = flip fmapCheap

{-# INLINE fmapCheap #-}
fmapCheap :: HasTimeline t => (a -> b) -> Event (Impl t) a -> Event (Impl t) b
fmapCheap f = pushCheap $ return . Just . f

{-# INLINE tagCheap #-}
tagCheap :: HasTimeline t => Behavior (Impl t) b -> Event (Impl t) a -> Event (Impl t) b
tagCheap b = pushAlwaysCheap $ \_ -> sample b

{-# INLINE mergeWithCheap #-}
mergeWithCheap :: HasTimeline t => (a -> a -> a) -> [Event (Impl t) a] -> Event (Impl t) a
mergeWithCheap = mergeWithCheap' id

{-# INLINE mergeWithCheap' #-}
mergeWithCheap' :: HasTimeline t => (a -> b) -> (b -> b -> b) -> [Event (Impl t) a] -> Event (Impl t) b
...
appendEvents :: (HasTimeline t, Monoid a) => Event (Impl t) a -> Event (Impl t) a -> Event (Impl t) a
appendEvents = alignWith $ mergeThese mappend

-- | Alias for 'headE'
{-# DEPRECATED onceE "Use 'headE' instead" #-}
onceE :: (HasTimeline t, MonadHold (Impl t) m, MonadFix m) => Event (Impl t) a -> m (Event (Impl t) a)
onceE = headE
